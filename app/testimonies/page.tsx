...
const tagColor: Record<string, string> = {
  ...
  Healing: "linear-gradient(135deg,#1F4D3D,#12302A)",
  Salvation: "linear-gradient(135deg,#B8531F,#1F4D3D)",
  Ministry: "linear-gradient(135deg,#12302A,#1B130E)",
  Restoration: "linear-gradient(135deg,#B8531F,#E8A33D)",
  Testimony: "linear-gradient(135deg,#1F4D3D,#B8531F)",
};
  ...
  return (
    <main>
      ...
      <section style={{ background: "var(--ink)", padding: "150px clamp(20px,5vw,64px) 96px", position: "relative", overflow: "hidden" }}>
        ...
        <div aria-hidden style={{ position: "absolute", bottom: -120, left: -100, width: 480, height: 480, background: "radial-gradient(circle,rgba(31,77,61,.18),transparent 65%)", pointerEvents: "none" }} />
              ...
              <Reveal key={i} delay={(i % 3) * 80}>
                <article style={{ height: "100%", background: "var(--paper)", border: "1px solid var(--line)", borderRadius: 24, padding: "28px 28px 26px", display: "flex", flexDirection: "column", boxShadow: "0 12px 30px rgba(27,19,14,.06)" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
                    <div style={{ display: "grid", placeItems: "center", width: 42, height: 42, borderRadius: 12, background: tagColor[s.tag] || "var(--ink)", boxShadow: "0 8px 18px rgba(31,77,61,.22)" }}>
