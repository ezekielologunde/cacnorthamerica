...
function shippingHtml(opts: {
  ...
  const trackingBlock = trackingNumber
    ? `<div style="background:#EEF4FF;border-radius:12px;padding:24px;margin-bottom:28px;text-align:center">
        ...
        <p style="font-size:22px;font-weight:800;color:#12141E;margin:0;letter-spacing:0.5px">${trackingNumber}</p>
      </div>`
    : `<div style="background:#F9F8F6;border-radius:12px;padding:18px 24px;margin-bottom:28px">
        <p style="font-size:14px;color:#12141E;margin:0">No tracking number was provided for this shipment. Your order ships within <strong>5–7 business days</strong>.</p>
      </div>`;
  ...
  const itemRows = lineItems
    .map(
      (li) =>
        `<tr>
          <td style="padding:10px 0;font-size:14px;color:#12141E;border-bottom:1px solid rgba(18,20,30,.07)">${li.description}</td>
          <td style="padding:10px 0;font-size:14px;color:#12141E;text-align:center;border-bottom:1px solid rgba(18,20,30,.07)">${li.quantity}</td>
          <td style="padding:10px 0;font-size:14px;color:#12141E;text-align:right;font-weight:600;border-bottom:1px solid rgba(18,20,30,.07)">${formatCents(li.amount_total, currency)}</td>
        </tr>`,
    )
    .join("");
...
<body style="margin:0;padding:0;background:#F9F8F6;font-family:Georgia,serif">
  <div style="max-width:600px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(18,20,30,.10)">
    ...
    <div style="padding:40px">
      ...
      <p style="font-size:16px;color:#12141E;line-height:1.75;margin:0 0 28px">
      ...
      ${
        lineItems.length > 0
          ...
          <thead>
            <tr>
              <th style="font-size:11px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:rgba(18,20,30,.4);text-align:left;padding-bottom:8px">Item</th>
              <th style="font-size:11px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:rgba(18,20,30,.4);text-align:center;padding-bottom:8px">Qty</th>
              <th style="font-size:11px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:rgba(18,20,30,.4);text-align:right;padding-bottom:8px">Total</th>
            </tr>
          </thead>
          <tbody>${itemRows}</tbody>
          <tfoot>
            <tr>
              <td colspan="2" style="padding:14px 0 0;font-size:14px;font-weight:700;color:#12141E">Order Total</td>
              <td style="padding:14px 0 0;font-size:16px;font-weight:800;color:#12141E;text-align:right">${formatCents(amountTotal, currency)}</td>
            </tr>
          </tfoot>
        </table>`
          : ""
      ...
      <div style="background:#F9F8F6;border-radius:12px;padding:20px 24px;margin-bottom:32px">
        <p style="font-size:14px;color:#12141E;line-height:1.75;margin:0">
          📦 <strong>Ships within 5–7 business days</strong><br>
          Questions about your order? Reach us on WhatsApp or email
          <a href="mailto:info@cacnorthamerica.com" style="color:#2D42C9;text-decoration:none">info@cacnorthamerica.com</a>.
        </p>
      </div>

      <hr style="border:none;border-top:1px solid rgba(18,20,30,.08);margin:0 0 24px">
      <p style="font-size:12px;color:rgba(18,20,30,.4);line-height:1.7;margin:0;text-align:center">
        <a href="https://www.cacnorthamerica.com" style="color:#2D42C9;text-decoration:none">cacnorthamerica.com</a>
      </p>
    </div>
  </div>
</body>
