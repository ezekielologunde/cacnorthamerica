...
export default async function StorePage({
  ...
  const categories = [
    { key: "convention", label: t("conventionApparel"), products: storeProducts.filter((p) => p.category === "convention") },
    { key: "good_women", label: t("goodWomenApparel"), products: storeProducts.filter((p) => p.category === "good_women") },
    { key: "youth", label: t("youthApparel"), products: storeProducts.filter((p) => p.category === "youth") },
    { key: "christian_education", label: t("materialsHeading"), products: storeProducts.filter((p) => p.category === "christian_education") },
  ];
      ...
      <section style={{ background: "var(--ink)", padding: "150px clamp(20px,5vw,64px) 70px" }}>
        <div style={{ maxWidth: 720, margin: "0 auto", textAlign: "center" }}>
          ...
          <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(38px,6vw,64px)", letterSpacing: "-0.03em", color: "#fff", margin: "16px 0 0", lineHeight: 1 }}>
            <RevealText immediate>{t("apparelHeading")}</RevealText>
          </h1>
          <Reveal delay={140}>
