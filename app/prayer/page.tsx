  ...
  return (
    <main>
      ...
      <section style={{ background: "var(--ink)", padding: "150px clamp(20px,5vw,64px) 90px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: -120, left: "50%", transform: "translateX(-50%)", width: 720, height: 480, background: "radial-gradient(circle,rgba(30,58,107,.32),transparent 65%)", pointerEvents: "none", animation: "gradient-drift 16s ease-in-out infinite" }} />
      ...
      <section style={{ background: "linear-gradient(135deg,var(--red-deep),var(--red))", padding: "clamp(56px,7vw,84px) clamp(20px,5vw,64px)", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: "-15%", background: "radial-gradient(circle at 75% 50%,rgba(61,95,161,.32),transparent 60%)", pointerEvents: "none", animation: "gradient-drift 14s ease-in-out infinite" }} />
      ...
      <section style={{ background: "var(--ink)", padding: "clamp(44px,5vw,70px) clamp(20px,5vw,64px)" }}>
        <Reveal style={{ maxWidth: 900, margin: "0 auto", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 24 }}>
          ...
          <Link href="/salvation" className="press btn-sheen" style={{ display: "inline-flex", alignItems: "center", gap: 9, background: "var(--red)", color: "#fff", fontWeight: 700, fontSize: 15, padding: "15px 28px", borderRadius: 999, textDecoration: "none", whiteSpace: "nowrap", flexShrink: 0, boxShadow: "0 10px 24px rgba(30,58,107,.35)" }}>
            Accept Christ today →
          </Link>
        </Reveal>
      </section>
