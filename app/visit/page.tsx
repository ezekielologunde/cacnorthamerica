...
export default function VisitPage() {
  ...
  const inputBase: React.CSSProperties = {
    ...
    background: "rgba(245,246,250,.07)",
    border: "1px solid rgba(245,246,250,.16)",
  ...
  const labelBase: React.CSSProperties = {
    ...
    color: "rgba(245,246,250,.45)", letterSpacing: "1px",
    ...
    <main>
      ...
      <section style={{ background: "var(--cream)", padding: "80px clamp(20px,5vw,64px)" }}>
        <div className="r2" style={{ maxWidth: 1100, margin: "0 auto", gap: 60 }}>
          <Reveal>
            <div style={{ height: 320, borderRadius: 20, overflow: "hidden", border: "1px solid var(--line)", marginBottom: 20, boxShadow: "0 10px 26px rgba(18,20,30,.06)" }}>
      ...
      <section style={{ background: "var(--ink)", padding: "clamp(60px,8vw,100px) clamp(20px,5vw,64px)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          <Reveal style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <p style={{ fontSize: 16, color: "rgba(245,246,250,.6)", margin: 0 }}>Whether it&apos;s your first visit or you&apos;re ready to join — we&apos;d love to connect with you.</p>
          </Reveal>

          <Reveal delay={80}>
            {submitted ? (
              <div style={{ textAlign: "center", padding: "56px 32px", background: "rgba(245,246,250,.06)", borderRadius: 24, border: "1px solid rgba(245,246,250,.12)" }}>
                ...
                <p style={{ fontSize: 15, color: "rgba(245,246,250,.6)", margin: 0 }}>We&apos;ll be in touch soon to connect you with a church near you.</p>
              </div>
            ) : (
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                    {VISIT_TYPES.map(t => (
                      <button key={t.value} type="button" onClick={() => setForm(p => ({ ...p, visitType: t.value }))}
                        style={{ padding: "10px 18px", borderRadius: 999, fontSize: 14, fontWeight: 700, fontFamily: "inherit", cursor: "pointer", border: form.visitType === t.value ? "none" : "1px solid rgba(245,246,250,.2)", background: form.visitType === t.value ? "var(--red)" : "transparent", color: form.visitType === t.value ? "#fff" : "rgba(245,246,250,.7)" }}>
                        {t.label}
                      </button>
                    ))}
                  </div>
                    ...
                    {GROUPS.map(g => {
                      ...
                      return (
                        <button key={g} type="button" onClick={() => toggleGroup(g)}
                          style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "9px 16px", borderRadius: 999, fontSize: 13.5, fontWeight: 700, fontFamily: "inherit", cursor: "pointer", transition: "all .15s", border: active ? "none" : "1px solid rgba(245,246,250,.2)", background: active ? "var(--gold)" : "transparent", color: active ? "#12141E" : "rgba(245,246,250,.7)" }}>
