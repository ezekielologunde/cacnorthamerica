  ...
  return (
    <main>
        ...
        <div style={{ maxWidth: 720, margin: "0 auto", textAlign: "center", position: "relative", zIndex: 2 }}>
          ...
          <Reveal delay={80}>
            <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(46px,6.5vw,90px)", letterSpacing: "-2px", color: "var(--ink)", margin: "16px 0", lineHeight: .92 }}>
              We&apos;d love to<br />
              <span style={{ color: "var(--red)" }}>hear from you.</span>
            </h1>
          </Reveal>
          <Reveal delay={160}>
      ...
      <section style={{ background: "var(--cream-2)", padding: "60px clamp(20px,5vw,64px) 100px" }}>
        <Reveal>
          <div style={{ maxWidth: 820, margin: "0 auto", background: "var(--paper)", borderRadius: 24, border: "1px solid var(--line)", padding: "clamp(28px,4vw,52px)", boxShadow: "0 16px 48px rgba(18,20,30,.06)" }}>
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} noValidate>
                ...
                {isMembershipSubject && (
                  <div style={{ background: "rgba(45,66,201,.06)", border: "1px solid rgba(45,66,201,.2)", borderRadius: 12, padding: "14px 18px", marginBottom: 16, display: "flex", alignItems: "flex-start", gap: 12 }}>
                ...
                <button
                  ...
                  style={{ padding: "14px 36px", borderRadius: 999, background: status === "loading" ? "var(--line)" : "linear-gradient(100deg,#F0742F,#2D42C9)", color: "#fff", fontWeight: 700, fontSize: 15, border: "none", cursor: status === "loading" ? "not-allowed" : "pointer", transition: "opacity .15s", letterSpacing: ".3px" }}
                >
