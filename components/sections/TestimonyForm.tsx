...
export function TestimonyForm() {
  ...
  const card: React.CSSProperties = {
    ...
    border: "1px solid var(--line)", boxShadow: "0 30px 70px rgba(18,20,30,.12)",
    ...
    return (
      ...
      >
        <motion.span
          ...
          style={{ display: "grid", placeItems: "center", width: 76, height: 76, borderRadius: "50%", background: "linear-gradient(140deg,var(--flame),var(--red))", margin: "0 auto 24px", boxShadow: "0 14px 34px rgba(45,66,201,.36)" }}
        >
    ...
    <form onSubmit={handleSubmit} noValidate style={card}>
      <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
        ...
        <motion.button type="submit" disabled={loading} className="btn-sheen press-lg" whileTap={reduce ? undefined : { scale: 0.97 }}
          style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--red)", color: "#fff", fontWeight: 800, fontSize: 16, fontFamily: "var(--font-body)", border: "none", padding: "18px 28px", borderRadius: 999, cursor: loading ? "wait" : "pointer", opacity: loading ? 0.7 : 1, boxShadow: "0 16px 34px rgba(45,66,201,.4)", width: "100%" }}>
